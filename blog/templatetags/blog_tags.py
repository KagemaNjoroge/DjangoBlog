...
def load_sidebar(user, linktype):
    ...
    if value:
        ...
        recent_articles = Article.objects.filter(status="p").select_related(
            "author", "category"
        ...
        most_read_articles = (
            ...
            .order_by("-views")[: blogsetting.sidebar_article_count]
        )
        ...
        commment_list = (
            Comment.objects.filter(is_enable=True)
            .select_related("author")
            .order_by("-id")[: blogsetting.sidebar_comment_count]
        )
        # 标签云 计算字体大小
...
def gravatar_url(email, size=40):
    ...
    if usermodels:
        ...
        if users_with_picture:
            ...
            non_default_users = [
                u
                for u in users_with_picture
                if u.picture != default_avatar_path
                and not u.picture.endswith("/avatar.png")
            ]
            selected_user = (
                non_default_users[0] if non_default_users else users_with_picture[0]
            )
            ...
            logger.info(
                "Using {} OAuth avatar for {} from {}".format(
                    avatar_type, email, selected_user.type
                )
            )
            return url
    ...
    for plugin in get_loaded_plugins():
        try:
            widget_data = plugin.render_position_widget(
                position=position, context=context, **kwargs
            )
            if widget_data:
                widgets.append(widget_data)
        except Exception as e:
            logger.error(
                f"Error rendering widget from plugin {plugin.PLUGIN_NAME}: {e}"
            )
...
def plugin_critical_head_resources(context):
    ...
    for plugin in get_loaded_plugins():
        try:
            ...
            logger.error(
                f"Error loading critical head resources from plugin {plugin.PLUGIN_NAME}: {e}"
            )
...
def plugin_head_resources(context):
    ...
    for plugin in get_loaded_plugins():
        try:
            ...
            logger.error(
                f"Error loading head resources from plugin {plugin.PLUGIN_NAME}: {e}"
            )
...
def plugin_body_resources(context):
    ...
    for plugin in get_loaded_plugins():
        try:
            ...
            logger.error(
                f"Error loading body resources from plugin {plugin.PLUGIN_NAME}: {e}"
            )
...
def plugin_widget(context, plugin_name, widget_type="default", **kwargs):
    ...
    if plugin and hasattr(plugin, "render_template"):
        try:
            ...
            logger.error(
                f"Error rendering plugin widget {plugin_name}.{widget_type}: {e}"
            )
