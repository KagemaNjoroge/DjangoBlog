...
class IndexView(OptimizedArticleQueryMixin, ArticleListView):
    ...
    def get_queryset_data(self):
        # 使用 Mixin 提供的优化查询方法
        return self.get_optimized_article_queryset().filter(type="a", status="p")
    ...
    def get_context_data(self, **kwargs):
        ...
        # 提供基础SEO数据
        context["seo_title"] = (
            f"{blog_setting.site_name} | {blog_setting.site_description}"
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        parent_comments = (
            Comment.objects.filter(
                article=self.object, parent_comment=None, is_enable=True
            )
            .select_related("author")
            .prefetch_related("comment_set__author")  # 预加载子评论及其作者
            .order_by("-id")
        )
        ...
        hooks.run_action(
            ARTICLE_DETAIL_LOAD, article=article, context=context, request=self.request
        )
...
class CategoryDetailView(SlugCachedMixin, OptimizedArticleQueryMixin, ArticleListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs["seo_description"] = (
            f"Browse {categoryname} all articles under this category, total {article_count} articles."
        )
...
class TagDetailView(SlugCachedMixin, OptimizedArticleQueryMixin, ArticleListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs["seo_description"] = (
            f"View all about {tag.name} articles, total {article_count} content."
        )
...
def fileupload(request):
    """
    This method requires you to write your own client to upload images; it only provides image hosting functionality.
    :param request:
    :return:
    """
